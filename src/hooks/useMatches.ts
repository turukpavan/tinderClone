...
export const useMatches = () => {
  ...
  const loadMatches = useCallback(async (showFullLoader = false) => {
    if (showFullLoader) {
      setLoading(true);
    } else {
      setRefreshing(true);
      ...
      const formattedMatches: MatchItem[] = (rawData || [])
        .filter(
          (
            ...
            return (
              item !== null && item !== undefined && item.user !== undefined
            );
          },
        )
        .map(item => {
          ...
          return {
            ...
            user: {
              ...
              profileImage:
                rawItem.user.profileImage || 'https://via.placeholder.com/150',
            },
          };
        });
