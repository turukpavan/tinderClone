...
export const useOtpVerification = (
  ...
  const handleSubmit = async (code: string) => {
    ...
    try {
      ...
      Alert.alert('Invalid code', getAuthErrorMessage(error));
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      console.log('getCurrentUserDoc error', error);
      Alert.alert(
  ...
  const handleResend = async () => {
    ...
    const result = await runWithAuthHandling(() => sendOtp(phoneNumber), {
  ...
  return { loading, resending, resendCooldown, handleSubmit, handleResend };
};
